...
let declare_abstraction ?(opaque = false) ?(continuation = default_continuation) ?kind arity evd env a name =
  ...
  let b = Retyping.get_type_of env evd a in
  let b_R = relation arity evd env b in
  let sub = range (fun k -> prime evd arity k a) arity in
  let b_R = EConstr.Vars.substl sub b_R in
  let a_R = fun evd ->
    let evd, a_R = translate arity evd env a in
    debug [`Abstraction] "a_R = " env evd a_R;
    debug_evar_map Debug.all "abstraction, evar_map = " env evd;
    evd, a_R
  in
  let hook =
    match EConstr.kind evd a with
      | Const cte when
          let cte = (fst cte, EInstance.kind evd (snd cte)) in
