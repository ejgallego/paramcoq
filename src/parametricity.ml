...
and translate_mind_param order evd env (l : Context.Rel.t) =
  ...
  List.map (function (Name x,c) -> (x, c) | _ -> failwith "anonymous param")
    (List.flatten (aux env [] (List.map fromDecl l)))
  ...
  let trans_consname s = translate_id order (Id.of_string ((Id.to_string name)^"_"^(Id.to_string s))) in
  {
    ...
    mind_entry_lc =
      begin
        ...
        let l =  List.map (fun x -> snd (decompose_prod_n_assum !evdr p x)) l in
        ...
        let for_each_constructor k =
          ...
          let third_part = range (fun m -> prime !evdr order m (apply_head_variables_ctxt (mkConstructU  ((ikn, k + 1), inst)) params)) order in
